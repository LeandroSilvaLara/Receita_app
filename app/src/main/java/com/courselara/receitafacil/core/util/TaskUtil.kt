...
abstract class FlowTask<in Parameters, Result : Any> {
    suspend operator fun invoke(parameters: Parameters): Flow<Result> = executeTakFlow(parameters)
    protected abstract suspend fun executeTakFlow(parameters: Parameters): Flow<Result>
}
